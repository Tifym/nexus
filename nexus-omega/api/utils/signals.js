...
class SignalEngine {
  ...
  calculateSignalScore(indicators, currentPrice, consensus) {
    ...
    if (score >= 50) { signal = 'STRONG_LONG'; confidence = Math.min(95, 65 + absScore - 50 - spreadPenalty); }
    else if (score >= 25) { signal = 'LONG'; confidence = Math.min(85, 50 + absScore - 25 - spreadPenalty); }
    else if (score <= -50) { signal = 'STRONG_SHORT'; confidence = Math.min(95, 65 + absScore - 50 - spreadPenalty); }
    else if (score <= -25) { signal = 'SHORT'; confidence = Math.min(85, 50 + absScore - 25 - spreadPenalty); }
