...
export class TradingEngine {
  ...
  async checkPositionExit(position, currentConsensus) {
    ...
    const trailingTrigger = position.notional * 0.015;
    if (unrealizedPnl > trailingTrigger) {
      const newStop = isLong ? currentPrice * (1 - this.config.trailingStopPercent) : currentPrice * (1 + this.config.trailingStopPercent);
      if ((isLong && newStop > position.stop_loss) || (!isLong && newStop < position.stop_loss)) {
        await supabase.rpc('move_stop_to_breakeven', { _position_id: position.id, _new_stop: newStop });
      }
    }

    const breakevenTrigger = position.notional * this.config.breakevenTrigger;
    if (!position.breakeven_moved && unrealizedPnl > breakevenTrigger) {
      const breakevenPrice = isLong ? position.entry_price * (1 + this.config.breakevenBuffer) : position.entry_price * (1 - this.config.breakevenBuffer);
      await supabase.rpc('move_stop_to_breakeven', { _position_id: position.id, _new_stop: breakevenPrice });
    }
  ...
  async closePosition(position, exitPrice, reason, consensus) {
    ...
    const exitFee = notional * this.config.tradingFee;
